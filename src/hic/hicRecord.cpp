...
BoolVec Record::mark_balanced_fields(
    const Scheme& sch, const Format& fmt, Record& rec,
    const XIndexVec& rank_to_def, size_t size )
{
    ...
    return m_base.mark_balanced_fields(
        sch, fmt, get_field_vec(), rec.get_field_vec(), rank_to_def, size );
}
